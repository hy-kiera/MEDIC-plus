...
def to_super_class_matrix(sub_matrix, super_class_sizes):
    ...
    for i in range(num_super_classes):
        for j in range(num_super_classes):
            sub_block = sub_matrix[
                start_indices[i] : end_indices[i], start_indices[j] : end_indices[j]
            ]
    ...
    for id in domain_split:
        if mode == "random":
            ...
            with torch.no_grad():
                ...
                group_matrix = to_super_class_matrix(
                    sub_matrix=class_matrix, super_class_sizes=group_length_list
                )
