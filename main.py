...
if __name__ == "__main__":
    ...
    classes_partition = split_classes(
        classes_list=known_classes, index_list=class_index_list, n=num_group
    )
    ...
    if net_name == "resnet18":
        ...
        net = muticlassifier(
            net=gfnet_fast("/data0/xiran/MEDIC-plus-vit/save/model/pretrain/gfnet-h-ti.pth"),
    ...
    if optimize_method == "SGD":
        ...
        scheduler = get_scheduler(
            optimizer=optimizer, instr=schedule_method, step_size=int(num_epoch * 0.8), gamma=0.1
        )
    ...
    if num_epoch_before != 0:
        ...
        for epoch in range(num_epoch_before):
            scheduler.step()
        logger.log_params(NumEpochBefore=num_epoch_before)
    ...
    for epoch in range(num_epoch_before, num_epoch):
        ...
        if (epoch + 1) % eval_step == 0:
            ...
            recall["va"], recall["ta"], recall["oscrc"], recall["oscrb"] = eval_all(
                net, val_k, test_k, test_u, log_path, epoch, device
            )
