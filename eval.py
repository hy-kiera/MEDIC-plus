...
if __name__ == "__main__":
    ...
    hits = args.hits
    save_dir = args.save_dir = os.path.join(args.save_dir, args.algorithm, args.dataset)
    ...
    for threshold in outlier_range:
        ...
        log(
            "Acc_k: {:.4f} Acc_u: {:.4f}, Acc: {:.4f}, H-Score: {:.4f} ({})".format(
                acc_k, acc_u, acc, hs, threshold
            ),
            log_path,
        )

    print(
        "Best OverallAcc: %.4f" % (best_overall_acc),
        "Best OverallHscore: %.4f" % (best_overall_Hscore),
    )
